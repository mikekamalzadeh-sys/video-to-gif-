...
def ask_conversion_type(chat_id):
    keyboard = {
        'keyboard': [[{'text': BTN_VIDEO_NOTE}, {'text': BTN_GIF}, {'text': BTN_VOICE}]],
...
def process_pending_video(chat_id, mode):
    ...
    labels = {'video_note': 'ویدیو مسیج', 'gif': 'گیف', 'voice': 'ویس'}
            ...
            if updates.get('ok') and updates.get('result'):
                for update in updates['result']:
                    ...
                    if 'video' in message:
                        ...
                        send_message(chat_id, '🎥 یه ویدیو برام بفرست تا به ویدیو مسیج، گیف یا ویس تبدیلش کنم، یا یه عکس بفرست تا به استیکر تبدیلش کنم.\nحداکثر حجم قابل قبول: ' + str(MAX_DOWNLOAD_MB) + ' مگابایت.')
            else:
                time.sleep(1)
